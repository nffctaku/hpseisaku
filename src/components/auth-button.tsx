...
export function AuthButton({ isMobile = false }: { isMobile?: boolean }) {
  ...
  if (isMobile) {
    return (
      <button onClick={handleSignIn} disabled={signingIn} className="hover:text-gray-300 w-full text-center disabled:opacity-60">
        {signingIn ? 'ログイン画面へ移動中…' : 'ログイン'}
      </button>
    );
  }
  return (
    <Button
      ...
      disabled={signingIn}
      variant="outline"
      className="h-[54px] w-[234px] rounded-[10px] border border-gray-300 bg-white text-[15px] font-bold text-gray-900 shadow-none hover:bg-gray-50 disabled:opacity-70"
    >
      {signingIn ? (
