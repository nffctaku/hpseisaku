...
export default async function PlayerDesignTestPage({
  ...
  const leagueStats = effectiveSeason
    ? await getLeaguePlayerStats(ownerUid, playerId, player, effectiveSeason)
    : { appearances: 0, minutes: 0, goals: 0, assists: 0 };
  const matchRows = effectiveSeason ? await getPlayerMatchResults(ownerUid, playerId, effectiveSeason) : [];
  const leagueCompetitionLabel = effectiveSeason ? await getLeagueCompetitionLabel(ownerUid, effectiveSeason) : null;
  const seasonCompetitionStats = effectiveSeason ? await getSeasonCompetitionStats(ownerUid, playerId, player, effectiveSeason) : [];
